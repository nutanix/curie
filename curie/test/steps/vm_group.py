...
from curie.test.steps.test import Wait
...
class CloneFromTemplate(BaseStep):
  ...
  def __init__(self, scenario, vm_group_name, power_on=False, annotate=False,
    ...
    if self.vm_group is None:
      raise CurieTestException("No VM Group named %s" % vm_group_name)
    self.description = "%s: Cloning from template" % self.vm_group.name()

  def verify(self):
    if not self.scenario.goldimages_directory:
      raise CurieTestException(
        "scenario.goldimages_directory must be set before running "
        "CloneFromTemplate (value: %r)" % self.scenario.goldimages_directory)
...
class PowerOff(BaseStep):
  ...
  def _run(self):
    ...
    if not vms:
      raise CurieTestException("Failed to power off %s: No VM(s) found" %
                                self.vm_group.name())
...
class PowerOn(BaseStep):
  ...
  def _run(self):
    ...
    vms = self.vm_group.get_vms()
    if not vms:
      raise CurieTestException("Failed to power on %s: No VM(s) found" %
                                self.vm_group.name())
    ...
    try:
      ...
      raise CurieTestException(
        "Not all VMs in VM Group %s accessible within %d seconds. Verify that "
        "all VMs received appropriate IP addresses and that communication "
        "between the X-Ray VM and workload VMs is possible." % (
          self.vm_group.name(), timeout_secs))
    if ret is None:
...
class Grow(BaseStep):
  ...
  def __init__(self, scenario, vm_group_name, count_per_node=None,
    ...
    if self.count_per_node is None and self.count_per_cluster is None:
      raise CurieTestException("Must specify either count_per_node or "
                                "count_per_cluster")
    if self.count_per_node is not None and self.count_per_cluster is not None:
      raise CurieTestException("Must specify count_per_node or "
                                "count_per_cluster, but not both")
  ...
  def verify(self):
    ...
    if new_count <= self.vm_group.total_count():
      raise CurieTestException("Must specify a new VM count (%d) greater "
                                "than what already exists (%d)" %
                                (new_count, self.vm_group.total_count()))
  ...
  def _run(self):
    ...
    if not existing_vms:
      raise CurieTestException("Cannot resize VM group %s: Must be cloned "
                                "from template first" % self.vm_group.name())
...
class RunCommand(BaseStep):
  ...
  def _run(self):
    ...
    vms = self.vm_group.get_vms()
    if not vms:
      raise CurieTestException("Failed to execute '%s' on %s: No VM(s) "
                                "found" % (self.command, self.vm_group.name()))
...
class MigrateGroup(BaseStep):
  ...
  def verify(self):
    ...
    try:
      ...
      raise CurieTestException(
        "Invalid to_node index '%s'. Cluster contains %s nodes (max index is "
        "%s)." %
