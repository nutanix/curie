...
class BaseResult(object):
  ...
  def parse(scenario, name, definition):
    ...
    from curie.result.vm_group_result import VMGroupResult
  ...
  def _add_expected_value_details(self, test_result):
    """
    Adds expected value details to the result.

    The expected value details consist of a single expected value as well as
    multiple bands that can illustrate things like prediction bands.

    Args:
      test_result (curie_test_pb2.CurieTestResult) The result to be updated.
    """

    result_expected_value = self.kwargs.get("result_expected_value", None)
    if result_expected_value:
      test_result.result_expected_value = float(result_expected_value)

    result_value_bands = self.kwargs.get("result_value_bands", [])
    for value_band in result_value_bands:
      band = curie_test_pb2.CurieTestResult.ValueBand()
      band.name = value_band["name"]
      band.upper = float(value_band["upper"])
      band.lower = float(value_band["lower"])
      test_result.result_value_bands.extend([band])
