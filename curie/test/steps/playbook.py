...
class Run(BaseStep):
  ...
  def __init__(self, scenario, filename, inventory, remote_user, remote_pass,
    ...
    if not os.path.isfile(self.filename):
      raise CurieTestException("Playbook %r does not exist." % self.filename)

    self.inventory = inventory
    self.remote_user = remote_user
    self.remote_pass = remote_pass
    self.description = "running playbook %s on %s" % (filename,
                                                      ", ".join(self.inventory))
  ...
  def _groups_to_dict(groups, scenario):
    ...
    for group in groups:  # type: str
      if group == "cvms":
        # The timeout for this step is 20 minutes! This is problematic when the
        # cluster is down and the cache needs to kick in.
        cvm_ips = [IPv4Address(unicode(vm.vm_ip()))
                   for vm in scenario.cluster.vms() if vm.is_cvm()]
        group_dict[group] = [str(cvm_ip) for cvm_ip in sorted(cvm_ips)]
        log.debug("Adding Ansible inventory group %r with hosts: %r", group,
                  group_dict[group])
      elif group == "nodes":
        node_ips = [IPv4Address(unicode(node.node_ip()))
    ...
    groups_missing_hosts = []
    for group, host_list in group_dict.iteritems():
      if not host_list:
        groups_missing_hosts.append(group)
    if groups_missing_hosts:
      raise CurieTestException("No hosts matching group %r found in %r."
                                % (groups_missing_hosts, scenario))
    return group_dict
  ...
  def _run(self):
    ...
    log.info("Running playbook %r on hosts %r as user %r with variables: %r",
             self.filename, group_dict, self.remote_user, self.variables)
    try:
    ...
    self.create_annotation("Playbook %r changed hosts %r." % (self.filename,
                           changed_hosts))
