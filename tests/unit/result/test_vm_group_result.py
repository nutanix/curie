...
class TestVMGroupResult(unittest.TestCase):
  ...
  @mock.patch(
    "curie.result.vm_group_result.prometheus.PrometheusAdapter")
  ...
  def test_get_result_pbs_iops(self, mock_prom_adapter):
    ...
    result_expected_value = 100
    mock_prom_adapter.return_value.get_disk_ops.return_value = data
    result = VMGroupResult(self.scenario, "fake_result", self.vm_group,
                           result_type="iops", result_hint="Some hint.",
                           result_expected_value=result_expected_value,
                           result_value_bands=result_value_bands)
    self.scenario.prometheus_address = "fake_address:1234"
    self.scenario.phase = Phase.RUN
    self.scenario._phase_start_time_secs[Phase.RUN] = 123456
    self.scenario._phase_start_time_secs[Phase.TEARDOWN] = 234567
    ...
    for pb in ret:
      ...
      self.assertEqual("Some hint.", pb.result_hint)
      self.assertEqual(float(result_expected_value), pb.result_expected_value)
      for idx, band in enumerate(result_value_bands):
        self.assertEqual(band["name"], pb.result_value_bands[idx].name)
        self.assertEqual(float(band["lower"]),
                         pb.result_value_bands[idx].lower)
        self.assertEqual(float(band["upper"]),
                         pb.result_value_bands[idx].upper)
